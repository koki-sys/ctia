...
exports.score = {
    addNamegame: async (score, userId, roomId) => {
        ...
        try {
            ...
            const sql =
                "INSERT INTO score (id, score, game_id, user_id, room_id) values (null, ?, 1, ?, ?)"
    ...
    getNamegame: async (roomId) => {
        ...
        try {
            ...
            const params = [roomId]
            const sql =
                "SELECT * FROM score LEFT OUTER JOIN user ON score.user_id = user.id WHERE game_id = 1 AND score.room_id = ? ORDER BY score DESC LIMIT 1"
