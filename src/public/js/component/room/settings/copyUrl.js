const copyUrl = (client, gameUrl) => {
    ...
    const displayQR = async (roomData) => {
        if (
            sessionStorage.getItem("roomCount") == 1 &&
            sessionStorage.getItem("limitPerRoom") == 1
        ) {
            $("#qr").html(
                "<strong class='text-primary'>一人だけなので<br>共有する必要はありません。</strong>",
            )
            ...
            $("#qr").html(
                "<strong class='text-danger'>QRコードの表示に<br>失敗しました。<br>設定し直してください。</strong>",
            )
        }
    }
